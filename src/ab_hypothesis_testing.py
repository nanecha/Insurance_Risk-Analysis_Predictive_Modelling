...
class ABHypothesisTesting:
    ...
    def test_risk_across_provinces(self):
        """
        Test if there are significant risk differences (TotalClaims)
        across provinces.
        Null Hypothesis: There are no risk differences across provinces.
        """
        province_groups = [
            self.data[self.data['Province'] == p]['Total_Claim']
    ...
    def test_risk_between_zipcodes(self):
        """
        Test if there are significant risk differences (Total Claims)
        between zip codes.
        Null Hypothesis: There are no risk differences between zip codes.
        """
        zipcode_groups = [
            self.data[self.data['Postacode'] == z]['TotalClaim']
            for z in self.data['Postacode'].unique()
        ]
    ...
    def test_margin_difference_between_zipcodes(self):
        """
        Test if there are significant margin (profit) differences
        between zip codes.
        Null Hypothesis: There are no significant margin 
        differences between zip codes.
        """
        self.data['Margin'] = self.data['Premium'] - self.data['TotalClaim']
        zipcode_groups = [
            self.data[self.data['Postacode'] == z]['Margin']
            for z in self.data['Postacode'].unique()
        ]
        result = f_oneway(*zipcode_groups)
        return {
    ...
    def test_risk_difference_gender(self):
        ...
        Test if there are significant risk differences (Total Claims) 
        between genders.
        Null Hypothesis: There are no significant risk differences between
        women and men.
        """
        male_group = self.data[self.data['Gender'] == 'Male']['TotalClaim']
        female_group = self.data[self.data['Gender'] == 'Female']['TotalClaim']
        result = ttest_ind(male_group, female_group, equal_var=False)
        return {
            "Test": "T-Test",
            "Null Hypothesis": (
                "No significant risk differences between women and "
                "men"
            ),
